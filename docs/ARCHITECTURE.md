...
1. `AutomaticCapturePolicy` 在后台入口检查固定开头口令；输入框、剪贴板、Fn 本机识别和应用入队分别守住此边界。待回答的问题不绕过检查，无口令文字不持久化、不发送 AI。手动输入 / 独立录音是主动提交；接收授权与 `NaturalTaskIntent` 的内容理解分开。
...
升级后旧的无口令自动队列会停止处理，只有用户手动核对并重试才能提交。
