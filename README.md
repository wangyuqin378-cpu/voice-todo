...
**Current source version: 0.1.1 / build36, in development.** macOS 26 only; Windows and Linux clients are not available yet, and there is no notarized download. Receiving transcripts from other voice tools is experimental.
...
A shortcut recorder in Settings lets you keep Fn or record the key / modifier combination used by your voice tool; the binding is shared by local recognition and experimental transcript reception. Recording settings pauses capture, and Esc cancels. System-reserved shortcuts and third-party compatibility still need device checks. Build36 fixes delayed reminder cancellation and stops obsolete scheduling work after a task is completed or cancelled.

[Product direction](docs/PRODUCT.md) · [Validation and remaining device checks](docs/REVIEW-BUILD36.md) · [Report an issue](https://github.com/wangyuqin378-cpu/voice-todo/issues)
